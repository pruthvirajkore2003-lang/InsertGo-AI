...
export type ProviderConfig = {
  ...
  baseUrl: string;
  /**
   * @deprecated No real API key ever belongs here: the app holds no LLM key at
   * all (the managed relay does). Kept only because the field is part of the
   * shape `GeminiProvider` reads; always "" or a dummy marker.
   */
  apiKey?: string;
  isDefault: boolean;
};

/** The one lane (SPEC §5.4): every request runs through the hosted InsertGo
 *  relay, which holds the key server-side. It lives here rather than in
 *  `aiProviders.ts` so the store can read it without pulling in the provider
 *  implementation. Nothing persists it — since BYOK was cancelled there is no
 *  provider list to store, so this constant IS the configuration. */
export const PROXY_CONFIG: ProviderConfig = {
  id: "backend",
  name: "Backend Proxy",
...
/** Where "Translate This" sends English text when the source names no target
 *  of its own. The skill bar is one click — there is nowhere to type a target
 *  at run time — so without a configured default the skill had no answer for
 *  the commonest input and refused. Free text: any language name the model
 *  understands works. */
export const DEFAULT_TRANSLATION_LANGUAGE = "Hindi";

export type Settings = {
  theme: ThemePreference;
  hotkey: string; // e.g. "Ctrl+`"
  defaultProviderId: string | null;
  /** Target language for "Translate This" when the source text names none and
   *  is already English. Fills the template's `[TARGET LANGUAGE]` slot; an
   *  explicit "Target language: X" line in the text still wins. */
...
export type ProviderRequest = {
  ...
  system?: string;
  /** Sampling temperature (SPEC §15.4: ≈0.3 for rewrites). Only lanes whose
...
export const DEFAULT_SETTINGS: Settings = {
  ...
  defaultTranslationLanguage: DEFAULT_TRANSLATION_LANGUAGE,
  selectionBar: true,
